...
const questions_reponses_generales = {
  "Do you think explanations, either graph, diagram or text-based, are useful ?":
    liste_answer_possible,
  "Which explanation format do you prefer ?": [
    "graph-based",
    "text-based",
    "bar chart",
  ],
  "Adding context to the explanations (neighborhood of the path’s nodes) is useful":
    liste_answer_possible,
  "Interacting with the other paths by using the Swap Path mode helps me understand better why this course was recommended to me.":
    liste_answer_possible,
  "Interacting with the other paths by using the Add Path mode helps me understand better why this course was recommended to me.":
    liste_answer_possible,
  "Choosing which predicates to display when exploring the path’s neighborhood is useful.":
    liste_answer_possible,
...
const updateGraph = (nodes, edges, infos) => {
  ...
  const nodeEnter = nodeSelection
    ...
    .each(function (d) {
      ...
      if (isRect) {
        ...
        text.attr("x", 0).attr("y", 0).attr("font-size", "10px");
      } else {
        node.attr("r", 8).attr("fill", getNodeColor(d.group));
      }
    })
    ...
    .on("mouseout", (event, d) => {
      ...
      if (d.label === d.id) {
        d3.select(event.currentTarget).select("rect").attr("stroke-width", 0);
      } else {
  ...
  const edgeLabelEnter = edgeLabelSelection
    .enter()
    .append("text")
    .attr("font-size", "9px")
    .attr("fill", "#555")
...
const loadPath = async () => {
  ...
  try {
    ...
    const courseResp = await fetch(
      `${API_URL}/api/random_course?start=${user.value}`
    );
    ...
    document.getElementById(
      "affichage-cours"
    ).innerHTML = `Cours recommandé : <strong>${course.value}</strong>`;
    ...
    const pathResp = await fetch(
      `${API_URL}/api/path?start=${user.value}&end=${course.value}&w=true&choix=false`
    );
    ...
    const res = await fetch(
      `${API_URL}/api/all_path?start=${user.value}&end=${course.value}&w=true&choix=false`
    );
    ...
    const top5res = await fetch(
      `${API_URL}/api/top5?user=${user.value}&course=${course.value}`
    );
...
function resetPredicats() {
  // Mettre à jour les prédicats (les enlever de la zone drop), mais aussi tout le reste (BUTTON REINITIALISER)
...
function focusOnNode(graph, nodeId, width, height) {
  ...
  graph.nodeGroup
    ...
    .attr("fill", (d) =>
      d.id === nodeId ? "limegreen" : getNodeColor(d.group)
    );
  ...
  graph.svg.transition().duration(750).call(graph.zoom.transform, newZoom);
}
      ...
      if (grandparent == 0) {
        ...
        fetch(`${API_URL}/api/voisins?voisin=${encodedNodeId}&choix=false`) // Récuperer ses voisins pour les retirer
          ...
          .then((data2) => {
            ...
            if (path2_en_cours) {
              // Si on a ajouté au chemin initial d'autres chemins (mode ajout ou échange), prendre en consideration le total des chemins
            ...
            data.edges = data.edges.filter(
              (e) =>
                ...
                !(
                  edgesToRemove.has(`${e.from}->${e.to}`) &&
                  !pathEdge.has(`${e.from}->${e.to}`)
                )
            );
